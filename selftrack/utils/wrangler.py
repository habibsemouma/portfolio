...
def activity_extract():
    ...
    for date in dates:
        if type(date)==str:
            df=sqldf(f"select * from activity_df where date='{date}' and name!='Screen off (locked)'")
            df["Hour"]=df["Time"].apply(convert_hour)
            df["Duration"]=pandas.to_timedelta(df["Duration"]).dt.total_seconds().apply(lambda x:np.round(x/60,2))
            df=sqldf("select Hour as hour,sum(Duration) as duration from df group by Hour")
...
def activity_by_app():
    ...
    df=sqldf("select name,sum(duration) as duration from df group by name").dropna().sort_values("duration")
